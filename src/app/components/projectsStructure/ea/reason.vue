<template>
    <el-dialog :visible="show_reason_modal"
      width="40%"
      :show-close="false"
      >
          <div slot="title">
            <button type="button" class="close" @click="cancelAction()">&times;</button>
          </div>  
              <div class="modal-body"  style="padding:25px !important" >
                 <div class="form-group">
                    <textarea  v-validate="'required'" name="reason"  v-model="reason.reason" class="form-control" placeholder="Why is this module being rejected?"></textarea>
                     <span align="center" style="color:red;font-size:15px" v-show="errors.has('reason')" class="help is-danger">A reason should be provided</span>
                 </div>
                 <div class="form-group">
                   <input  v-validate="'required'"  name="extendedTime" v-model="reason.extendedTime" class="form-control" type="number" placeholder="how many hours should be added? " min="1"></input>
                   <span align="center" style="color:red;font-size:15px"  name="time" v-show="errors.has('extendedTime')" class="help is-danger">More hours should be added to the module</span>
                 </div>
               <div class="form-group" align="right">
                   <button class="btn btn-primary" @click="submitReason()">Okay</button>
               </div>
                </div>
          </div> 
          </div>
...
props: ["show_reason_modal"],
methods:{

       
    closeModal(){
      var self = this;
      this.$emit("closeReasonModal",self.reason);
      this.reason = {};
      // alert();
    } , 
    submitReason(){
