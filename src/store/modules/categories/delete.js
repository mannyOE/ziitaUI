...
const state = {
  sub: {
    error: false,
    successMsg: 'Category was deleted successfully.',
    loading: false,
    loadingId: null,
    unauthorized: null,
    unverified: null,
...
// actions
const actions = {
  deleteCategory ({ dispatch, commit, state }, args) {
    commit('loading', args.id);

    return api.deleteCategory(args)
    .then((result) => {
      if (result.error === undefined) {
        // commit('success');
        // dispatch('categories/getCelebCategories', store.state.auth.sub.token, {root: true}).then(function () {
        // });
        commit('clearErrors');
