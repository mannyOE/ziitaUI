...
const getters = {
  ...
  hireables: state => state.sub.hireables,
...
// actions
const actions = {
  getHireables ({ dispatch, commit, state }, dargs) {
    // Loading
    ...
    api.getUnhired(dargs)
    .then((result) => {
      if (result.error === undefined) {
        commit('clearErrors');
        // Use response data
        const data = result.data
        commit('setHireables', data);
      } else {
...
const mutations = {
  ...
  setHireables (state, hireables) {
    state.sub.hireables = hireables || []
  },
